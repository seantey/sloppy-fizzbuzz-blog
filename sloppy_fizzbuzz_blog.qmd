...
![](assets/ugly_llama_v3.webp){width=85% fig-align="center"}
...
For this project, let's focus on the well-known children's game and basic coding interview question: **[FizzBuzz](https://en.wikipedia.org/wiki/Fizz_buzz)**.
...
![](assets/drake_LoRA_meme_v3.webp){width=88% fig-align="center"}
...
$\mathrm{Reward}(f_x)$ =
$\mathrm{ugliness\_score}$ −
$\mathrm{penalty\_gates}$ +
...
```python
result=[] ← E: missing spaces (should be result = [])
for x in range(1,n+1): ← N: bad name 'x', E: missing spaces
    tmp=x%15 ← N: ambiguous 'tmp', E: missing spaces
    if tmp==0:l.append("FizzBuzz") ← N: ambiguous 'l', E: multiple statements, missing spaces
```

Ugliness is the first component of our reward function, but we need more than just that because if you design the reward poorly, you don’t just add noise, you add bad signals that get exploited.
...
So to counteract that, we penalize the model heavily whenever it generates code patterns that we don't like. (Enforced via code checks in reward function, see Section 6.4 of the [Jupyter notebook](https://github.com/seantey/sloppy-fizzbuzz/blob/main/ugly_fizzbuzz_grpo.ipynb))
...
**🥖 The BAGUETTE Score** a.k.a “**B**ag-based **A**ggregated **G**eometric **U**niqueness **E**ncoded-**T**oken **T**hreshold **E**valuator” score. 
...
This is just a fancy looking math equation I made up because I needed to justify the amount of time I banged my head against the wall trying to create a "novelty" component. I needed something that could exert the right type of exploration pressure so that the LLM could learn to generate more diverse solutions that are ugly.
...
For actual implementation, refer to section 6 of the [Jupyter notebook](https://github.com/seantey/sloppy-fizzbuzz/blob/main/ugly_fizzbuzz_grpo.ipynb) or read below for a breakdown of the core components.

**Tokenizer IDs**

I considered using [embeddings](https://huggingface.co/spaces/hesamation/primer-llm-embedding?section=what_are_embeddings?) because that's the natural thing to try in the context of LLMs. Guess what is the cosine embedding distance between regular FizzBuzz and ugly FizzBuzz? Very similar! And that makes sense, they are both Python code snippets for FizzBuzz and the differences in code structure aren't captured or differentiated well enough, at least with the usual vector distance measures that I tried: cosine, euclidean, etc.

But what if we want to try a frequency based distance measure for text such as bag distance, then why not leverage the token IDs used during model training? Instead of using a bag of words, we [tokenize](https://huggingface.co/learn/llm-course/en/chapter2/4) the text so that we get a similar perspective as the LLM but we just use the associated token IDs instead.
...
Here are some examples of what the model generated in a good training run ([notebook with outputs](https://github.com/seantey/sloppy-fizzbuzz/blob/main/ugly_fizzbuzz_300s_good_run.ipynb)):
...
![](assets/robot_gold_v2.webp){width=88% fig-align="center"}

## Links

If you enjoyed reading this, follow me on Twitter/X: [@seansrr](https://x.com/seansrr). Send me a message or leave a comment to let me know what you like or anything I can improve on!

**Check out these Github repos:**

- Sloppy FizzBuzz Repo: [github.com/seantey/sloppy-fizzbuzz](https://github.com/seantey/sloppy-fizzbuzz)
- Unsloth (LLM Fine-tuning & RL): [github.com/unslothai/unsloth](https://github.com/unslothai/unsloth)
- OpenEnv (RL interface and environments): [github.com/meta-pytorch/OpenEnv](https://github.com/meta-pytorch/OpenEnv)
